...
%% \newcommand{\cccast}[2]{\ensuremath{{#1}\,\left\langle{#2}\right\rangle}}
...
\title{The Holy Grail of Gradual Security \\
  {\small PL Wonks Seminar - Logic Seminar, Spring 2024} \vspace{-15pt}}
\author{Tianyu Chen}
...
% Outline:
% 1. Background
%   - The intuition of noninterference and the gradual guarantee
%   - There is a tension between the two properties
% 2. LambdaIFCStar in action:
%   - LambdaIFCStar embeds static IFC (the static extreme)
%   - LambdaIFCStar embeds dynamic IFC (the dynamic extreme)
%   - Gradual security typing in LambdaIFCStar
%       (by removing annotations from the static extreme)
%   - The tension (in an example) and how LambdaSecIFCStar solves it
%   - Type-based reasoning in LambdaIFCStar
% 3. The semantics of LambdaIFCStar
%   - Security coercion calculci
%   - The cast calculus LambdaIFCc
%   - Compilation from LambdaIFCStar to LambdaIFCc
...
\begin{frame}
  ...
  \begin{center}
    \includegraphics[width=4in]{galahad}
  \end{center}

  \footnotetext{\textit{The Achievement of the Grail}. Tapestries by Morris \& Co, 1890. Birmingham Museum and Art Gallery}
...
\begin{frame}
  \frametitle{Information-Flow and Noninterference}

  Consider boolean negation. Can we infer \texttt{output} from \texttt{input}?
  \[
  \key{let} \; \key{output} = \neg \key{input}
  \]
  ...
  \onslide<3-> {
  \item Requires witnessing at least two executions:
    {\footnotesize $\key{input}=\key{true}, \key{output}=\key{false}$} and
    {\footnotesize $\key{input}=\key{false}, \key{output}=\key{true}$}
  \item An information-flow property is a hyperproperty: a predicate on sets of executions
  \item Noninterference: \footnote{Specifically, termination-insensitive noninterference (TINI) for a functional programming language}
    two successful executions of a program produce the same value for different input.
  }
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    \item Security labels. Label input as \high; output as \low.
    \item[] Track and check the security labels
    \item IFC in a programming language, traditionally
      \begin{itemize}
      \item \bluetext{\ul{Static:}} using a type system
      \item \redtext{\ul{Dynamic:}} using runtime monitoring
      \end{itemize}
    \item A \textit{gradual} security programming language
      \begin{itemize}
      \item embeds both \bluetext{static} and \redtext{dynamic} IFC
      \item enables seamless transition between static and dynamic
      \item security label annotations
        \(
        \left\{
        \begin{minipage}[c]{0.5\linewidth}
          \item[] specific: $\low, \high$
          \item[] statically unknown: \unk
          \end{minipage}
        \right.
        \)
    \end{itemize}
...
\begin{frame}[containsverbatim]
  \frametitle{Review: \bluetext{Static IFC} Using a Type System}

  Modeling input and output:
  \begin{lstlisting}[mathescape,autogobble,numbers=none]
    user-input : $\Unit_{\low}$ → $\Bool_{\high}$    publish : $\Bool_{\low}$ → $\Unit_{\low}$
  \end{lstlisting}
  ...
  \begin{itemize}
  \item[\yes] \textcolor{Green}{Well-typed} \qquad \textit{and} \qquad
  ...
  \item Why? The return value of \texttt{fconst} is
    ...
    \right.
    \)
  \item No runtime check!
  \item IFC enforced by the type system alone
  \end{itemize}
\end{frame}


\begin{frame}[containsverbatim]
  \frametitle{Guarding Against Illegal \ul{Explicit Flows}, \bluetext{Statically}}

  Consider another \bluetext{fully static} program:
  ...
  \begin{itemize}
  \item[\no] \textcolor{Red}{Ill-typed.} Why?
  \item \textit{Illegal} \ul{explicit flow} from the \high-security input
    to \texttt{fid}
  \begin{itemize}
    \item \texttt{fid} expects \low argument
  \end{itemize}
  \item[\ding{82}] Program rejected by type-checker. Illegal explicit flow ruled out
    \bluetext{at compile time}
  \end{itemize}
\end{frame}
