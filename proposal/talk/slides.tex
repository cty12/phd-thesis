...
\begin{frame}
  ...
  \item The technical development of \Surface
  \item The gradual guarantee and noninterference for \Surface
  \end{itemize}
\end{frame}
...
%%%%%%%%%% Defaults in LambdaIFCStar %%%%%%%%%%%
\begin{frame}[fragile]
  \frametitle{Too Many Annotations?}
  ...
  \begin{lstlisting}[mathescape,autogobble,numbers=none,xleftmargin=.15\textwidth,xrightmargin=.15\textwidth]
    let x = private-input () in
...
\begin{frame}
  \frametitle{Technical Development}

  We define a gradual IFC calculus \Surface with mutable
  references, first-class functions, and conditionals.

  \begin{itemize}
  \item The semantics of \Surface is by translation $\mathcal{C}$ to a cast
    calculus \CC
  \item Casts are represented by coercions on types (a la Henglein) and
  ...
  \begin{array}{rcl}
    \ell & \in & \{\low,\high\}  \\
    g & \in & \{\low,\high,\unk\} \\
    M & ::=  & x \MID \const{k}{\ell} \MID \lam{g}{x}{A}{M}{\ell} \MID \app{M}{M}{p} \\
      & \MID & \ifexp{M}{M}{M}{p} \\
      & \MID & \refexp{\ell}{M}{p} \MID \deref{M}{p} \MID \assign{M}{M}{p} \\
    N & ::=  &  \ldots \MID \ccaddr{n} \MID \cccast{N}{c} \MID \blame{p} \MID \ccprot{\mathit{e}}{\ell}{M}{} \\
    c & ::= & \coerc{\id{\iota}}{s} \MID \coerc{(\refco{c}{d})}{s} \MID \coerc{\funco{s}{c}{c}}{s}  \\
    s & ::= & \id{g} \MID \up \MID \inj{\ell} \MID \proj{\ell}{p} \MID \bot^{\bl{p}} \MID s \seq s \\
    e & ::= & \ell \MID \blame{p} \MID \cccast{e}{s}
  \end{array}
  \]}
...
\begin{frame}
  \begin{theorem}[Gradual guarantee for \Surface]
    ...
    \end{align*}
    then there exists a value $V$ and heap $\mu$ such that
    \begin{align*}
      &\compile{M}\mid\emptyset\mid\low \longrightarrow^{*} V\mid\mu \\
      &V \sqsubseteq V'
    \end{align*}
...
\begin{frame}
  \begin{theorem}[Noninterference for \Surface]
    Suppose program $M$ is well-typed: $(x{:}\Bool_{\high}) ; \low \vdash M : \Bool_{\low}$. \\
    If for any boolean inputs $b_1,b_2$,
    {\normalfont
      \begin{gather*}
        (\compile{M})[x := b_1] \mid \emptyset \mid \low \longrightarrow^{*} V_1 \mid \mu_1 \\
        (\compile{M})[x := b_2] \mid \emptyset \mid \low \longrightarrow^{*} V_2 \mid \mu_2
    \end{gather*}}
    then $V_1 = V_2$.
  \end{theorem}
\end{frame}
...
\begin{frame}
  ...
  \item Gradual information flow can be nicely expressed using coercions, which
    is the first step towards an efficient implementation.
  \item The Agda mechanization of \Surface, the proof of the gradual
    guarantee, and some of the noninterfence proof are available at
    \begin{center}
