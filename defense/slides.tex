...
\begin{frame}
  \begin{quote}
    ...
    information flow control (IFC). This belief requires one to assume that,
    even though the most popular variable names are \lq thing\rq~and \lq
    thing2,\rq~programmers will magically become disciplined software
    architects when confronted with a Dungeons-and-Dragons-style type
    system...''
...
\begin{frame}[containsverbatim]
  ...
  value that does not depend on \texttt{priv} or \redtext{\textcircled{2}
...
\begin{frame}
  ...
  \vspace{10pt}
  A \Surface program is a \Surface term that is well-typed at $(x{:}\Bool_{\high});\low\vdash M : \Bool_{\low}$.
\end{frame}
...
\begin{frame}
  ...
  \footnotesize
  \begin{array}{rcl}
    c     & ::=  & \id{g} \MID \up \MID \inj{\ell} \MID \proj{\ell}{p} \MID \bot^{\bl{p}} \\
    \bar{c} & ::=  & \id{g} \MID \err{g_1}{g_2}{p} \MID \bar{c} \seq c \\
    ...
    M & ::=  & x \MID \ccconst{k} \MID \ccaddr{n} \MID \cclam{x}{M}\MID \cclet{x}{M}{A}{M}  \\
                     & \MID & \ccapp{M}{M}{A}{B}{\ell} \MID \ccappstar{M}{M}{A}{T} \\
                     & \MID & \ccif{M}{A}{\ell}{M}{M} \MID \ccifstar{M}{T}{M}{M} \\
                     & \MID & \ccref{\ell}{M} \MID \ccrefproj{\ell}{M}{p} \MID \ccderef{M}{A}{\ell} \MID \ccderefstar{M}{T} \\
                     & \MID & \ccassign{M}{M}{T}{\ell}{\ell} \MID \ccassignproj{M}{M}{T}{g}{p} \\
...
\begin{frame}
  ...
  \item \Surface: a gradual IFC calculus
  \item Technical development
  \item[\ding{43}] Meta-theoretical results
    \begin{itemize}
      \item Type safety for \Surface
      \item Gradual guarantee for \Surface
      \item Noninterference for \Surface
...
\begin{frame}
  ...
  \begin{theorem}[Type safety of \Surface]
    If $M$ is a \Surface program and $\mathit{eval}(M,b)=r$ \\ then the
    evaluation result is well-typed $\vdash r : \Bool_{\low}$.
  \end{theorem}
\end{frame}
