...
before we check and reject the flow from \high through \unk to \low.
...
\begin{definition}[Security level of a coercion]
  \label{def:sec}
...
\raggedright
  ...
  \begin{array}{rcll}
    ...
    \text{terms}  & L , M , N & ::=  & x \MID \ccconst{k} \MID \ccaddr{n} \MID \cclam{x}{N} \MID
                           ...
                           &   & \MID & \ccprot{e}{\ell}{M}{A} \MID \cccast{M}{\bm{c}} \MID \blame{p} \\
...
The typing rules for the static variants are similar to the typing rules in a
static security type system. For example, in the static function application
rule ${\vdash}\textit{app}$, both top-level labels on the function type ($\ell$
and $\ell' \curlyvee \ell$) as well as the security label that the current PC is
typed at ($\ell'$) are static and the rule mirrors one in a static system. On
the other hand, in the dynamic version of application rule
${\vdash}\textit{app}\star$, both top-level labels as well as the label of the
co-domain type are \unk and PC is allowed to be typed at \unk, indicating the
presence of injections. As another example, in the static version of memory
assignment rule ${\vdash}\textit{assign}$, all labels to perform the heap policy
check, including the security of the memory address itself ($\ell$), the
security of the memory cell that the address references ($\hat{\ell}$), and the
security of PC ($\ell'$) are known statically and satisfy $\highlightblue{\ell'
  \curlyvee \ell \preccurlyeq \hat{\ell}}$. At runtime, this static assignment
can happen directly without any runtime overhead (as is shown in the example of
Section~\ref{sec:example2}). Its dynamic counterpart rule
${\vdash}\textit{assign?}$ does not maintain these static security invariants
and thus requires runtime NSU checking, which is implemented as a projection on
PC.

As we shall see in the reduction rules, the semantics of the protection term
\texttt{prot} performs two things: (1) \texttt{prot} promotes the security of
the value reduced from its body by level $\ell$ (2) it uses a new PC to reduce
its body. However, the new PC cannot be any label expression. It has to be one
with higher security than both the current PC and the security level $\ell$. We
capture this invariant in side condition $\ell' \curlyvee \ell \preccurlyeq
|\PC|$ in rule ${\vdash}\textit{prot}$, where $\ell'$ is the security of the
current PC and $|\PC|$ is the security for the new PC. The invariant is used in
the proof of noninterference (case \textit{prot-ctx} in the proof of
Lemma~\ref{lem:sim-leq}, which is used by Theorem~\ref{thm:NI-Surface}).
...
(Figure~\ref{fig:comp-stampc}) in a similar way. The value is either (1) a raw
value or (2) a coercion-wrapped value. Suppose the value is a raw value, if its
type has \low security and is stamped with \high, the value becomes wrapped with
a subtype coercion (the second case of \textit{stamp} in
Figure~\ref{fig:stamp-val}); otherwise, the value stays unchanged (the first and
the third cases of \textit{stamp}). If the value is wrapped with an irreducible
coercion, we stamp the top-level coercion sequence (the fourth case of
\textit{stamp}).

\paragraph{Function Application.} The $\beta$ rule is standard for IFC languages.
...
\paragraph{NSU and heap operations.} Let us consider reference creation
...
\begin{figure}
  \fbox{$\reducemult{M}{\mu_1}{\PC}{N}{\mu_2}$}
  \begin{gather*}
...
The multi-step reduction of \CC is defined as the reflexive transitive closure
