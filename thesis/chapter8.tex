...
We presented the design of a gradual information-flow
language \Surface that satisfies both noninterference and the gradual
guarantee while maintaining the principle of type-based reasoning. The
key to the design of \Surface is to walk back the decision in \GSLRef
to include the unknown label \unk among the runtime security labels.
So \Surface takes a more standard approach to gradually-typed IFC:
the \unk label can be used in type annotations but not as the security
level of a runtime value.  The \Surface language is defined by
translation to a cast calculus \CC.  This intermediate language
employs a coercion calculus to express the implicit conversions
between more-or-less precise parts of the program. We proved that
\Surface satisfies termination-insensitive noninterference.
We proved that \Surface satisfies the gradual guarantee and
mechanized the result in Agda.
