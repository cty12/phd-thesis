...
{\color{NavyBlue}   %%%% new text
  ...
  \begin{figure}[tbp]
    \raggedright
        {\small \[
          \begin{array}{rcll}
            \text{evaluation result}     & r & ::=  & b \MID \mathit{error}\;\bl{p} \MID \mathit{diverge}
          \end{array}
          \]}
        \fbox{$\mathit{eval}(M, b)=r$}
        \begin{align*}
          \mathit{eval}(M, b) =&\,b'\;&\text{if}\;(\compile(M))[x:=\ccconst{b}] \Downarrow b' \\
          \mathit{eval}(M, b) =&\,b'\;&\text{if}\;(\compile(M))[x:=\ccconst{b}] \Downarrow \cccast{b'}{\mathbf{c}} \\
          \mathit{eval}(M, b) =&\, \mathit{error}\;\bl{p} \;&\text{if~}\;(\compile(M))[x:=\ccconst{b}] \Downarrow \blame{\bl{p}} \\
          \mathit{eval}(M, b) =&\, \mathit{diverge} \;&\text{if~}\;(\compile(M))[x:=\ccconst{b}] \Uparrow
        \end{align*}
...
well-typed \Surface program $(x{:}\Bool_{\high}) ; \low \vdash M : \Bool_{\low}$
