...
(PC) is cast to the security level of the memory location to write. If
successful, the projection justifies the heap policy that the memory location to
write is at least as secure as PC. As a result, each memory location must also
be associated with a specific security label, so that the target of a cast for
NSU is always specific. Therefore, I came up with a new IFC language
...
in \S\ref{sec:def-dynifc} and compare \DynIFC to dynamic IFC languages in the
literature in \S\ref{sec:compare-dynifc}.
...
secure as the current PC (rule \textit{ref?-ok}). If the NSU check
...
information. Consider the example:
\begin{lstlisting}[style=tt]
...
is of high, and we are assigning to low memory, so the NSU check fails, thereby
...
I choose the label in the syntax of reference creation as the security of the
memory location so that \Surface satisfies the gradual guarantee. Imagine a
gradual IFC language that supports \unk as an annotation on reference creation.
The term \refexp{\colorbox{highlight}{\unk}}{\true}{} would create a new memory
location whose security is decided at run time by the current PC and store the
value \true~in it, similar to the semantics of \laminfo. Consider the following
program:
...
branch condition is injected, so NSU checking happens during assignment. On line
4, PC is promoted to high by the branch condition. The program assigns to a high
memory location, $\high \preccurlyeq \high$, so the NSU check is successful. On
line 6, the program dereferences address \texttt{a}, resulting in a high boolean
...
shows that the security of a memory location must be fixed in the syntax of
reference creation, as opposed to being decided by PC.
