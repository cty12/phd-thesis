...
level. \DynIFC is particularly useful for the noninterference result in
Chapter~\ref{ch:noninterference}, whose proof is by simulation between \CC and
\DynIFC. We are going to compare \DynIFC with other dynamic IFC languages in the
...
Figure~\ref{fig:dyn-ifc}. When creating a reference, we need to perform a
runtime NSU check $\pc \preccurlyeq \ell$, so that the security of the memory
location to write to is at least as high as the current PC (rule
\textit{ref?-ok}, highlighted). If the check fails, the program errors (rule
\textit{ref?-fail}). In this way, programs such as
\[
\texttt{publish (! (if (input ()) (ref low false) (ref low true)))}
\]
will error at runtime due to NSU, guarding against illegal implicit information
flows through the heap.
