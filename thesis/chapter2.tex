...
%% Contrary to the claim by~\textcite{Amorim:2020aa}, a gradual IFC programming
%% language can satisfy the gradual guarantee while supporting type-guided
%% classification.
%
In this chapter, I present \Surface, which is the first language that satisfies
both noninterference and the gradual guarantee while supporting type-guided
classification. Previously, \textcite{Toro:2018aa} designed \GSLRef, a language
that satisfies noninterference and supports type-guided classification but
sacrifices the gradual guarantee. \textcite{Amorim:2020aa} designed GLIO, a
language that satisfies noninterference and the gradual guarantee but sacrifices
type-guided classification. In addition, \textcite{Amorim:2020aa} identify the
exclusion of type-guided classification as the reason that GLIO satisfies the
gradual guarantee and \GSLRef does not.
%% There are two
%% major differences between \GSLRef by \textcite{Toro:2018aa} and GLIO by
%% \textcite{Amorim:2020aa}, where the former satisfies noninterference but not the
%% gradual guarantee and the latter satisfies both. First, \GSLRef supports
%% type-guided classification, but GLIO does not. Second, \GSLRef includes the
%% unknown security label \unk in runtime security labels, but GLIO does not.

\Surface satisfies noninterference and the gradual guarantee while supporting
type-guided classification because it (1) excludes \unk from runtime labels and
(2) uses security coercions to represent casts. Before I designed \Surface, I
followed GLIO and created \SurfaceOld~\parencite{Chen:2022aa}. Like GLIO and
unlike \Surface, the language \SurfaceOld does not use coercions as the cast
representation and does not support typed-guided classification. During the
development of \SurfaceOld, I realized that NSU checking could be modeled using
a sequence of casts, which could be expressed as coercions. The source of the
first coercion and the target of the last coercion must be specific (\low or
\high) no matter how long the sequence is. The key to designing a gradual IFC
language is to identify the source of an injection and the target of a
projection. Injection happens when a value is cast from some specific security
label to \unk. A constant must be associated with a specific security label so
that when the constant is injected, its label becomes the source of the
injection. Implicit flow causes the program counter (PC) to be injected when
branching on an injected branch condition. Projection, on the other hand,
happens when an injected value is cast to a specific security label. NSU
checking is a projection: the PC is cast to the security level of the memory
location to write to. If successful, the projection justifies the heap policy
that the memory location is at least as secure as PC. As a result, each memory
location should also be associated with a specific security label. By excluding
\unk from runtime labels and using coercions as the cast representation, I came
up with a new IFC language \Surface~\parencite{chen2024quest}. Like GLIO and
unlike \GSLRef, \Surface satisfies the gradual guarantee. Like \GSLRef and
unlike GLIO, \Surface supports type-guided classification, so \Surface enjoys
the same type-based reasoning capabilities through free theorems as \GSLRef.
...
in \S\ref{sec:surface-def}. After that, I put \Surface into action in
\S\ref{sec:examples}. I present example programs to demonstrate how
...
the same time. Finally, in \S\ref{sec:embedding}, I conclude the chapter
...
\raggedright
  ...
  \begin{array}{rcll}
    ...
    \text{raw types}                & T, S      & ::= & \iota \MID \Fun{A}{g}{B} \mid \Refer{(T_g)} \\
    \text{types}                    & A, B      & ::= & T_g \\
  \end{array}
  \]}
...
\emph{security labels}. A function type $(\Fun{A}{g_2}{B})_{g_1}$ carries an
additional security label $g_2$, which is the type of the program counter (PC) to
...
Figure~\ref{fig:type-prec} gives the definition of precision on types.
...
%{\color{NavyBlue}   %%%% new text
  ...
  \begin{figure}[tbp]
    \raggedright
        \[
          \begin{array}{rcll}
            \text{evaluation result}     & r & ::=  & k \MID \mathtt{fun} \MID \mathtt{addr} \MID \mathtt{diverge} \MID \mathtt{stuck}
          \end{array}
        \]
        \fbox{$\mathit{obs}(V)=r$}
        \begin{align*}
          ...
          \mathit{obs}(\cclam{x}{N}) =&\, \mathtt{fun} \\
          \mathit{obs}(\cccast{(\cclam{x}{N})}{\bm{c}}) =&\, \mathtt{fun} \\
          \mathit{obs}(\ccaddr{n}) =&\, \mathtt{addr} \\
          \mathit{obs}(\cccast{(\ccaddr{n})}{\bm{c}}) =&\, \mathtt{addr}
        \end{align*}
        \fbox{$\mathit{eval}(M, b)=r$}
        \begin{align*}
          \mathit{eval}(M, b) =&\, \mathit{obs}(V)\;&\text{if}\;(\compile{M})[x:=\ccconst{b}] \mid \emptyset \Downarrow V \mid \mu \\
          \mathit{eval}(M, b) =&\, \mathtt{diverge} \;&\text{if~}\;(\compile{M})[x:=\ccconst{b}] \mid \emptyset \Downarrow \blame{\bl{p}} \mid \mu \\
                                                    & &\text{or}~ (\compile{M})[x:=\ccconst{b}] \mid \emptyset \Uparrow \\
          \mathit{eval}(M, b) =&\, \mathtt{stuck}\;&\text{otherwise}
        \end{align*}
...
Chapter~\ref{ch:compile}. The compilation function is of the form $\compile{M} =
...
Chapter~\ref{ch:sem}. The evaluation function is of the form
$\mathit{eval}(M,b)=r$. It takes a well-typed \Surface term $(x{:}\Bool_{\high})
; \low \vdash M : A$ and a boolean constant $b \in \{\true, \false\}$ as input.
The evaluation of $M$ either produces the observation of a value
$\mathit{obs}(V)$, or diverges, or gets stuck. We treat runtime errors as a
special case of \texttt{diverge}. This is because in the context of IFC, one may
not want blame information to be observable in a production system as it could
reveal information. Instead, we force the program to diverge whenever blame
is detected and send a private error message to the software developer for
debugging purposes.
...
\begin{definition}[Whole programs of \Surface]
  \label{def:surface-program}
  A \Surface program is a \Surface term that is well-typed at
...
due to the first type annotation on line 2, and the third cast projects to \low
due to the second type annotation on line 2:
\[
\cccast{
  \cccast{
...
security labels in all the types are statically known; the type system alone
...
In the dynamic extreme of \GSLRef, all security labels in all the types are
unknown (\unk). The dynamic extreme of \GSLRef is a dynamic IFC programming
language named \DynIFC. In the dynamic extreme, every memory location is
associated with a fixed security level. \DynIFC is particularly useful for the
noninterference proof in Chapter~\ref{ch:noninterference}. The proof is by
simulation between \CC and \DynIFC. In the rest of this section, I present the
formal definition of \DynIFC in \S\ref{sec:def-dynifc} and compare \DynIFC to
dynamic IFC languages in the literature in \S\ref{sec:compare-dynifc}.
...
\preccurlyeq \ell$ so that the memory location to write to is at least as secure
as the current PC (rule \textit{ref?-ok}); if the NSU check fails, the program
errors (rule \textit{ref?-fail}). In this way, programs such as
...
$\pc \curlyvee \ell \preccurlyeq \hat{\ell}$ (rule \textit{assign?-ok}); if the
NSU check fails, the program errors (rule \textit{assign?-fail}). The NSU check
on assignment is equivalent to $\pc \preccurlyeq \hat{\ell}$ and $\ell
\preccurlyeq \hat{\ell}$. The first part ($\pc \preccurlyeq \hat{\ell}$) is
similar to the check on reference creation; it ensures that the security level
($\hat{\ell}$) of the memory location to write to is at least as high as the
current PC ($\pc$). The second part ($\ell \preccurlyeq \hat{\ell}$) ensures
that the memory location is at least as secure as the address itself. The second
part is necessary because choosing which address to assign to may leak
information.
%
\pagebreak
...
let a2 = ref low false in
  ...
  publish (! a1)
\end{lstlisting}
\noindent On line 3, we assign to low memory and PC is low, so the first part of
the check succeeds. To prevent leaking information through the heap, we must
also check the security of the address against the security of the memory
location. In the example above, the address is high because the branch condition
is of high, and we are assigning to low memory, so the NSU check fails. The
program errors, thereby preventing information leak.
...
literature, specifically \laminfo~\parencite{austin2009efficient} and
LIO~\parencite{STEFAN:2017ta}.

\subsubsection{Comparing \DynIFC to \laminfo}

\DynIFC is different from \laminfo, because the security of a memory location is
fixed during program execution in the former but can change in the latter.
%
In \DynIFC, the security level of a memory location is decided by the programmer
during reference creation and remains unchanged throughout program execution. In
\laminfo, on the other hand, the security level of a memory location is decided
by the current PC during reference creation and can increase monotonically as
the program runs.

I choose to specify the security of the memory location in the syntax of
reference creation so that \Surface satisfies the gradual guarantee. Imagine a
gradual IFC language that supports \unk as an annotation on reference creation,
in which the term \refexp{\colorbox{highlight}{\unk}}{\true}{} would create a
new memory location whose security is decided at run time by the current PC and
store the value \true~in it (analogous to the semantics of \laminfo). Consider
...
\noindent This program runs successfully. On line 3, the branch condition is
injected, so NSU checking happens during assignment. On line 4, PC is promoted
to high by the branch condition. The program assigns to a high memory location,
$\high \preccurlyeq \high$, so the NSU check is successful. On line 6, the
program dereferences address \texttt{a}, resulting in a high boolean that is the
negation of the input.

If we change the annotation on the reference creation (line 2) and make the
security of the memory location decided by PC at runtime:
...
created. The memory cell stores \texttt{true}. The branch condition is injected,
so NSU checking happens on line 4 to ensure that the security of the memory
location is at least as high as that of PC. The program attempts to write to low
memory under high PC, so the NSU check fails and signals a runtime error. The
more precise program runs to completion but the less precise one errors, which
violates the gradual guarantee. This counterexample of the alternative design
shows that to satisfy the gradual guarantee, the security of a memory location
must be specified in the syntax of reference creation, as opposed to being
decided by PC.

\subsubsection{Comparing \DynIFC to LIO}

\DynIFC is similar to LIO in that the security of a memory location stays the
same during program execution. However, compared with \DynIFC, LIO supports an
additional feature: first-class labels. When creating a reference in LIO, the
security of the new memory location may come from a security-label value that is
produced at runtime. In comparison, in \Surface the programmer has to specify
this security level during development. Implementing first-class labels in
\Surface is left for future work.
