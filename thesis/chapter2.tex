...
As discussed in Chapter~\ref{ch:intro}, there are two major differences between
\GSLRef~\parencite{Toro:2018aa} and GLIO~\parencite{Amorim:2020aa}: (1) \GSLRef
supports type-guided classification while GLIO does not; (2) \GSLRef includes
the unknown security label \unk in runtime security labels while GLIO does not.
In particular,~\textcite{Amorim:2020aa} identify the exclusion of type-guided
classification as the reason that GLIO is able to reconcile noninterference and
the gradual guarantee, while \GSLRef only satisfies noninterference but not the
gradual guarantee. I started the journey of \Surface by retrofitting GLIO with
vigilance and blame tracking, while leaving out type-guided classification and
excluding \unk from runtime security labels.
...
\todo[inline]{Define a down arrow and an up arrow in Chapter 3 and get rid of the
  reduction relation in eval}
...
\begin{align*}
  ...
  \mathit{eval}(M) =&\,\text{diverge}\;&\text{if~}\forall L.\reducemult{(\compile{M})}{\emptyset}{\low}{L}{\mu} \text{~and~} \exists N\,\mu'.\,\reduce{L}{\mu}{\low}{N}{\mu'}
\end{align*}
...
(\unk). The dynamic extreme of \GSLRef is a new dynamic IFC programming language
named \DynIFC, where every memory location has a fixed security level. I present
the syntax and semantics of \DynIFC in Figure~\ref{fig:dyn-ifc}. \DynIFC is
different from a typical dynamic IFC language in the literature (such as
$\lambda^{\textit{info}}$ of~\textcite{austin2009efficient}), in that each
memory location of \DynIFC is labeled with a fixed security level. This security
level is given by the programmer and remains unchanged during program execution.
I will show in Chapter~\ref{ch:mech-metatheory} that this choice of the dynamic
extreme makes it possible that \Surface satisfies the gradual guarantee.
...
\todo[inline]{Relate the heap design to the reduction rules}
\todo[inline]{Talk about prot}

%% \begin{align*}
%%     \small
%%     \compile{x}=\;&x \\
%%     \compile{\const{k}{\ell}}=\;&\dynconst{k}{\ell} \\
%%     \compile{\lam{\unk}{x}{A}{N}{\ell}}=\;&\dynlam{x}{\compile{N}}{\ell} \\
%%     \compile{\app{L}{M}{p}}=\;&(\compile{L})\;(\compile{M}) \\
%%     \compile{\ifexp{L}{M}{N}{p}}=\;&\dynif{(\compile{L})}{(\compile{M})}{(\compile{N})} \\
%%     \compile{\assign{L}{M}{p}}=\;&\dynassign{(\compile{L})}{?}{(\compile{M})} \\
%%     \compile{(\letexp{x}{M}{N})}=\;&(\dynlet{x}{\compile{M}}{\compile{N}}) \\
%%     \compile{\refexp{\ell}{M}{p}}=\;&\dynref{?}{\ell}{(\compile{M})}
%% \end{align*}
