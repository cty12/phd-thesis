...
I started the journey of designing the ideal gradual IFC language by
retrofitting GLIO with vigilance~\parencite{Gierczak:2024ab}. There are two
major differences between \GSLRef~\parencite{Toro:2018aa} and
GLIO~\parencite{Amorim:2020aa}, where the former satisfies noninterference but
not the gradual guarantee and the latter satisfies both. First, \GSLRef supports
...
\textcite{Amorim:2020aa} identify the exclusion of type-guided classification
(the first difference) as the reason that GLIO is able to reconcile
noninterference and the gradual guarantee. Following \textcite{Amorim:2020aa}, I
created a language called \SurfaceOld~\parencite{Chen:2022aa}. Like GLIO,
...
errors when the casts applied to a value are inconsistent while GLIO does not.
In gradually-typed languages, vigilance is a good property because it detects
inconsistencies between type annotations in different parts of a program. These
inconsistencies have to be fixed in order for the program to be statically
typed.

During the development of \SurfaceOld, I realized that including \unk in runtime
labels (the second difference) should instead be the root cause of the tension
between noninterference and the gradual guarantee. NSU checking can be expressed
as a sequence of casts. The source of the first cast and the target of the last
cast must be specific (\low or \high) no matter how long the sequence is.
Consequently, the key to designing a gradual security-typed language is to
identify the source of an injection and the target of a projection. Injection
happens when a value is cast from some specific security label to \unk. A
constant must be associated a specific security label so that when the constant
is injected, its label becomes the source of the injection. Implicit flow causes
the program counter (PC) to be injected when branching on an injected branch
condition. Projection, on the other hand, happens when an injected value is cast
to a specific security label. NSU checking is a projection: the program counter
(PC) is cast to the security level of the memory location to write to. If
successful, the projection justifies the heap policy that the memory location is
at least as secure as PC. As a result, each memory location must also be
associated with a specific security label, so that the target of a cast for NSU
is always specific. Therefore, I came up with a new IFC language
\Surface~\parencite{chen2024quest}. Like GLIO and unlike \GSLRef, \Surface
satisfies the gradual guarantee. Like \GSLRef and unlike GLIO, \Surface supports
both vigilance and type-guided classification, so \Surface enjoys the same
type-based reasoning capabilities through free theorems as \GSLRef.
...
special case of \textit{diverge}. This is because in the context of IFC, one may
