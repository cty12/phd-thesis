...
theorem of an IFC system is noninterference~\cite{goguen1982security}.
...
We explain how the type system approach defends against illegal explicit and
implicit information flows using examples. Consider two IO functions:
\texttt{private-input} and \texttt{publish}: the former returns a high-security
boolean that represents sensitive input information; the latter takes a
low-security boolean and publishes it into a publicly visible channel. Consider
the program with an illegal explicit flow from private input to public output:
\begin{verbatim}
...
develop the idea and support higher-order functions. There are research projects
that combine IFC type systems with other language features, such as bytecode
intermediate languages \autocite{barthe2005non}, object-oriented languages
