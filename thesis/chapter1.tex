...
satisfies the gradual guarantee, \Circled{3} enjoys type-guided classification,
and \Circled{4} utilizes NSU checking to enforce implicit flows through the heap
...
%%   runtime security levels (\S~\ref{sec:examples}).
%% \item A coercion calculus for security labels (\S~\ref{sec:cexpr})
%%   and a coercion calculus for secure values (\S~\ref{sec:coercions}).
%%   The two coercion calculi serve as our runtime IFC monitor.
%%   Our paper is the first work to apply the coercion calculus to an IFC setting.
%% \item A cast calculus \CC with IFC that defines the dynamic semantics of
%%   \Surface (\S~\ref{sec:cast-calculus}).  The proofs of (1) the
%%   gradual guarantee (\S~\ref{sec:gg}), (2) compilation
%%   from \Surface to \CC preserves types (\S~\ref{sec:compile}),
%%   and (3) type safety of \CC (\S~\ref{sec:type-safety}) are
%%   mechanized in Agda.
%% \item A proof of noninterference for \Surface
%%       (\S~\ref{sec:NI-Surface}) through the simulation between its cast calculus
%%       \CC and a dynamic IFC programming language (\S~\ref{sec:NI}).
%% \item The first design of a gradual security-typed language with type-guided classification
%%   that satisfies the gradual guarantee (\S~\ref{sec:surface-to-cc}).
%% \item We mechanize the proof of the gradual guarantee in the Agda proof assistant.
%% \end{itemize}
