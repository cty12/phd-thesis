...
\begin{itemize}
  ...
  \item In Chapter~\ref{ch:compile}, I define a type-preserving compilation from
    \Surface to \CC, so that the semantics of \Surface is given by \CC.
  \item In Chapter~\ref{ch:type-safety}, I prove type safety for \Surface. The
    type safety proof is mechanized in the Agda proof assistant.
  \item In Chapter~\ref{ch:gg}, I prove the gradual guarantee for \Surface. The
    proof for gradual guarantee is also mechanized in Agda.
  ...
  %%   support type-guided classification.
    %% The noninterference of \CCOld can be
    %% proved using the standard erasure technique.
    %% From \SurfaceOld I derive a
...
The Agda code of the mechanization of \Surface is available at:
\begin{center}
  \url{https://github.com/Gradual-Typing/LambdaIFCStar}
  %\quad at release \texttt{v2.0 (PLDI 2024)}
\end{center}
