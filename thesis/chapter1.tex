...
gradual guarantee relates the runtime behavior of programs that differ only in
the precision of type annotations.
...
A ideal gradual security-typed language (1) should enforce security by
satisfying noninterference (2) should satisfy the gradual guarantee
(3) should provide type-based reasoning through vigilance and
type-guided classification (4) should not require addition static
...
\Surface, which (1) enforces information flow security, (2)
satisfies the gradual guarantee, (3) enjoys type-based reasoning through
free theorems, and (4) utilizes NSU checking to enforce implicit flows
...
\item I design two coercion calculi that serve as the runtime IFC monitor:
  (1) a coercion calculus for security labels and (2) a coercion
